...
<div class="container" style="max-width: 550px">
  <form name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data"
        class="default-form" id="contact-form">
    <?= bitrix_sessid_post() ?>
    ...
    <? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>
          ...
          switch ($INPUT_TYPE):
            ...
            case "N":
              for ($i = 0; $i < $inputNum; $i++) {
                ...
                $max = '10';
                $isRequired = false;
                if (is_numeric($propertyID)) $type = 'number';
                if ($propertyID == 8) $max = 150;
                $isRequired = $propertyID == "NAME" || $propertyID == 8;
                ?>
                <input <? if ($isRequired) echo 'required'; ?>
                  placeholder="<?= $title; ?>" class="rounded"
                  type="<?= $type; ?>"
                  name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]"
                  size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>"
                  value="<?= $value ?>" min="0" max="<?= $max; ?>"/><br/><?
      ...
      <? if ($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0): ?>
        ...
        <input type="text" name="captcha_word" maxlength="50"
               placeholder="<?= GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT") ?>*" value="">
      <? endif ?>
    ...
    <div class="col-md-12 col-sm-12">
      <input id="submit" disabled type="submit" class="btn mt-30" name="iblock_submit"
